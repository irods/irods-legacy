...
#Output
#  Names of files that are missing the requied ACL
...
#Get USER_ID for the input user name
  ...
  *Userid = "";
  foreach(*GenQOut0) {msiGetValByKey(*GenQOut0,"USER_ID", *Userid);}
  if(*Userid == "") {
    writeLine("stdout","Input user name *User is unknown"); fail;}
#Get DATA_ACCESS_DATA_ID number that corresponds to requested access control
  ...
  foreach(*GenQOut1) {msiGetValByKey(*GenQOut1,"TOKEN_ID",*Access);}
...
INPUT *Coll = "/tempZone/home/rods/sub1", *User="rods", *Acl = "own"
OUTPUT ruleExecOut
