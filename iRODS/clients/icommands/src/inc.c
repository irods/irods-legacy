...
"in a time series. The time dimenion that ties these files together is assumed",
"to have a case insensitive name 'time' containing the number of seconds",
...
"time order must be generated using the 'inc --agginfo -w myCollection'",
"command. Each NETCDF file's datatype must be 'netcdf' type (e.g., using",
"'isysmeta mod datatype' command) for it to be recognized as part of the",
"aggregate file pool. If a file in the collection is not a 'netcdf' datatype,",
"it is excluded from the aggInfo while the the generation of aggInfo continues.",
"Then a NETCDF_INVALID_DATA_TYPE error is returned after the operation finished.",
"Once the .aggInfo file has been generated, the 'aggregate collection' path",
"will be treated as a single NETCDF file spanning the entire time series",
"by many of the NETCDF APIs and this command.",
" ",
