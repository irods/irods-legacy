...
"logically and presents to users as a single file. The time dimension must",
"have a case insensitive name 'time' containing the number of seconds since",
"the Epoch in values. Access of these NETCDF files can be achieved through",
"the TDS and ERDDAP drivers of iRODS once the URL of these files have been",
"registered with iCAT (see examples in https://www.irods.org/index.php/NETCDF)",
"This command can then be used to incrementally archive these time series",
"data to an iRODS 'aggregate collection'. An 'aggregate collection' is a",
"collection containing multiple NETCDF files in a time series tied together",
...
"a startingTimeIndex of 0 specifies that the new archive starts from the",
"beginning of the time series and a value of 999 specifies the archive starts",
"from the end. If this option is not specified, an incremental backup is",
"assumed from an existing archive.",
" ",
