...
class RODSPacket 
{
  ...
  public function toXML()
  {
    ...
    /*
    $dom = new DOMDocument();
    $root=$dom->createElement($this->type);
    foreach($this->packlets as $key => $val)
    {
      if (is_a($val,'RODSPacket'))
      {
        $root->appendChild($val->toDOMNode());
      }
      $root->appendChild($dom->createElement($key,$val));
    }
  ...
  */
  ...
  public function toDOMElement(DOMDocument $doc)
  {
    ...
    {
      if (is_a($packlet,'RODSPacket')) //if node is a packet
      {
      ...
      if (is_array($packlet)) //if node is an array
      {
        if (isset($packlet))
        {
          ...
          {
            if (is_a($sub_packlet,'RODSPacket')) //if sub_node is a packet
            {
            ...
            else
            {
              ...
              $node->appendChild($doc->createElement($name,$sub_packlet));
            } 
          }
        }
      }
      ...
      {
        $node->appendChild($doc->createElement($name,$packlet));
      }
    }
