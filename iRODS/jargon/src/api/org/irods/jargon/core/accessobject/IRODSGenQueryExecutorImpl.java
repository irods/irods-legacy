...
/**
 * Classic API adaptation of access object to process 'iquest-like' queries.
 * This service is usable but should be considered 'early access' and subject to
 * API changes.
 * 
 * @author Mike Conway - DICE (www.irods.org)
 * 
 */
public class IRODSGenQueryExecutorImpl extends AbstractIRODSAccessObject
	 ...
	 */
	protected IRODSGenQueryExecutorImpl(final IRODSCommands irodsCommands)
			throws JargonException {
		super(irodsCommands);
	}
	...
	public IRODSQueryResultSet getMoreResults(
		...
		return executeTranslatedIRODSQuery(
				irodsQueryResultSet.getTranslatedIRODSQuery(), 1);
	}
	...
	public IRODSQueryResultSet executeIRODSQuery(final IRODSQuery irodsQuery,
			final int numberOfRecordsDesired) throws JargonException,
	...
	private IRODSQueryResultSet executeTranslatedIRODSQuery(
		...
		GenQueryInp genQueryInp = GenQueryInp.instance(translatedIRODSQuery,
				continueIndex);
		Tag response = getIrodsCommands().irodsFunction(genQueryInp);

		if (response == null) {
			log.info("null response from IRODS call indicates no rows found, translated query was:"
					+ translatedIRODSQuery);
			int continuation = 0;
			List<IRODSQueryResultRow> result = translateResponseIntoResultSet(
					response, translatedIRODSQuery);
		...
		if (log.isDebugEnabled()) {
			log.debug("query reult for translated query:"
					+ translatedIRODSQuery);
		...
		return IRODSQueryResultSet.instance(translatedIRODSQuery, result,
				continuation);

	}

	List<IRODSQueryResultRow> translateResponseIntoResultSet(
