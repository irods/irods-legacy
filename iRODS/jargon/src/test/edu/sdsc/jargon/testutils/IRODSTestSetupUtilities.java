 ...
 * 
 * TODO: NB on windows, the current path is affected by the current path, look at ways to detach by better
 * setting of cwd
 * 
...
public class IRODSTestSetupUtilities {
	...
	public void clearIrodsScratchDirectory() throws TestingUtilsException {
		...
		try {
			...
			if (ilsResult.indexOf("/"
				...
				irmCommand.setObjectName(invocationContext.getIrodsScratchDir());
	...
	public void initializeIrodsScratchDirectory() throws TestingUtilsException {
		...
		imkdirCommand.setCollectionName(testingProperties
	...
	/**
	 * Create a directory under scratch with the given name, which is typically a name
	 * assigned per Junit test class
	 * @param testingDirectory <code>String</code> with a directory to go underneath scratch, do not supply leading '/'
	 * @throws TestingUtilsException
	 */
	public void initializeDirectoryForTest(String testingDirectory)
			throws TestingUtilsException {
		StringBuilder scratchDir = new StringBuilder();
		scratchDir.append(testingProperties.getProperty(IRODS_SCRATCH_DIR_KEY));
		scratchDir.append('/');
