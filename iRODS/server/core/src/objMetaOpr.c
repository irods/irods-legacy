...
modExeInfoForRepeat(rsComm_t *rsComm, char *ruleExecId, char* pastTime, 
    ...
    ***/
    ...
    if (status < 0) {
        rodsLog (LOG_ERROR,
          "regExeStatus: rsRuleExecMod/rsRuleExecDel Error of id %s failed, status = %d",
          ruleExecId, status);
    }
    else {
...
dataObjInfo_t **dataObjInfo, int writeFlag)
{
    ...
    if (status < 0) {
#if 0
        if (*dataObjInfo != NULL && getErrno (status) == ENOENT) {
#endif
        if (*dataObjInfo != NULL) {
...
#if 0
    if (cachedSpecColl->class == MOUNTED_COLL) {
	dataObjInfo_t *myDataObjInfo;

	myDataObjInfo = (dataObjInfo_t *) malloc (sizeof (dataObjInfo_t));
        memset (myDataObjInfo, 0, sizeof (dataObjInfo_t));
	rstrcpy (myDataObjInfo->objPath, dataObjInp->objPath, MAX_NAME_LEN);
	rstrcpy (myDataObjInfo->rescName, cachedSpecColl->resource, 
	  NAME_LEN);
        myDataObjInfo->specColl = cachedSpecColl;
	rstrcpy (myDataObjInfo->subPath, dataObjInp->objPath, MAX_NAME_LEN);
	rstrcpy (myDataObjInfo->dataType, "generic", NAME_LEN);
	memset (&rodsObjStat, 0, sizeof (rodsObjStat));
	status = specCollSubStat (rsComm, cachedSpecColl, dataObjInp->objPath,
	  &rodsObjStat, myDataObjInfo->filePath, &myDataObjInfo->rescInfo);
        if (status < 0) {
	...
	for (i = 0; i < NumStructFileType; i++) {
		...
		dataObjInfo_t *tmpDataObjInfo;
                ...
                memset (&myDataObjInp, 0, sizeof (myDataObjInp));
                rstrcpy (myDataObjInp.objPath, specCollMeta->collInfo1,
		  MAX_NAME_LEN);
		if (writeFlag > 0) {
		    status = getDataObjInfo (rsComm, &myDataObjInp,
		      dataObjInfo, ACCESS_DELETE_OBJECT, 1);
		} else {
                    status = getDataObjInfo (rsComm, &myDataObjInp,
                      dataObjInfo, ACCESS_READ_OBJECT, 1);
		}
		...
		rstrcpy (specColl->objPath, specCollMeta->collInfo1,
		  MAX_NAME_LEN);
		rstrcpy (specColl->phyPath, tmpDataObjInfo->filePath, 
		  MAX_NAME_LEN);
                tmpDataObjInfo = *dataObjInfo;
                tmpDataObjInfo->specColl = specColl;
                rstrcpy (tmpDataObjInfo->subPath, dataObjInp->objPath,
                  MAX_NAME_LEN);
                /* replicate specColl */
                tmpDataObjInfo = tmpDataObjInfo->next;
                while (tmpDataObjInfo != NULL) {
                    tmpDataObjInfo->specColl = malloc (sizeof (specColl_t));
                    *tmpDataObjInfo->specColl = *specColl;
                    rstrcpy (tmpDataObjInfo->subPath, dataObjInp->objPath,
                      MAX_NAME_LEN);
		    rstrcpy (specColl->phyPath, tmpDataObjInfo->filePath, 
	...
	if (i >= NumStructFileType) {
        ...
        memset (&myDataObjInp, 0, sizeof (myDataObjInp));
        rstrcpy (myDataObjInp.objPath, cachedSpecColl->objPath,
          MAX_NAME_LEN);
        if (writeFlag > 0) {
            status = getDataObjInfo (rsComm, &myDataObjInp,
              dataObjInfo, ACCESS_DELETE_OBJECT, 1);
        ...
        /* fill in DataObjInfo */
        while (tmpDataObjInfo != NULL) {
            tmpDataObjInfo->specColl = cachedSpecColl;
            rstrcpy (tmpDataObjInfo->subPath, dataObjInp->objPath,
              MAX_NAME_LEN);
	    if (tmpDataObjInfo == *dataObjInfo) {
                rstrcpy (cachedSpecColl->phyPath, tmpDataObjInfo->filePath,
                  MAX_NAME_LEN);
	    }
        }
