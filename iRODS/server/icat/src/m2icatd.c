...
#ifdef CAT_POSTGRES
char nextValStr[]="nextval('R_ObjectID')"; /* Postgres format */
char currValStr[]="currval('R_ObjectID')"; /* Postgres format */
#endif
#ifdef CAT_ORACLE
char nextValStr[]="%s.nextval"; /* Oracle format */
char currValStr[]="%s.currval"; /*/ Oracle format */
#endif
#ifdef CAT_MYSQL
...
/* Perl version has a begin and commit around these, but now I'm
 thinking it is better not to do that and just do a commit at the
	  ...
	  if (doDataInsert) {
		...
		wval = fprintf(FO, "insert into r_data_main (data_id, coll_id, data_name, data_repl_num, data_version, data_type_name, data_size, resc_name, data_path, data_owner_name, data_owner_zone, data_is_dirty, create_ts, modify_ts) values ((select %s), (select coll_id from r_coll_main where coll_name ='%s'), '%s', '%s', ' ', '%s', '%s', '%s', '%s', '%s', '%s', '1', '%s', '%s');\n",
			       nextValStr,
			       v_collection,
		...
		wval = fprintf(FO, 
			       "insert into r_objt_access ( object_id, user_id, access_type_id , create_ts,  modify_ts) values ( (select %s), (select user_id from r_user_main where user_name = '%s'), '1200', '%s', '%s');\n",
			       currValStr,
			       v_owner,
			       nowTime,
			       nowTime);
