...
int icListColl (rsComm_t *rsComm, collInp_t *listCollInp, collOprStat_t **collOprStat) {
	...
	if (isTrashPath (listCollInp->collName) == False &&
	  getValByKey (&listCollInp->condInput, FORCE_FLAG_KW) != NULL) {
            rodsLog (LOG_NOTICE,
              "icListColl: Recursively removing %s.",
...
int _icListColl (rsComm_t *rsComm, collInp_t *listCollInp, collOprStat_t **collOprStat) {
	...
	rstrcpy ((*collOprStat)->lastObjPath, listCollInp->collName,
           MAX_NAME_LEN);
    }
    return (status);
}
...
/*
int 
svrUnregColl (rsComm_t *rsComm, collInp_t *listCollInp)
{
    int status;
#ifdef RODS_CAT
    collInfo_t collInfo;
    ...
    status = getAndConnRcatHost (rsComm, MASTER_RCAT, listCollInp->collName,
                                &rodsServerHost);
    if (status < 0) {
       return(status);
    }
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        rstrcpy (collInfo.collName, listCollInp->collName, MAX_NAME_LEN);
        if (getValByKey (&listCollInp->condInput, IRODS_ADMIN_RMTRASH_KW)
          != NULL) {
	...
	addKeyVal (&listCollInp->condInput, UNREG_COLL_KW, "");
        status = _rcRmColl (rodsServerHost->conn, listCollInp, &collOprStat);
