// Sat Mar  1 23:26:30 PST 2008 do7!!!
...
int intGetDataObjAVUsVol7 (collInp_t * ptrInpColl,
			   UserDefinedMetadata_t * aAVUarray, int *iTotalAVUs,
			   char *strOut, ruleExecInfo_t * rei);
int
...
int msiChkDataAvuAttrOneValueOnly (msParam_t *coll, msParam_t * inpParam2, msParam_t * inpParam3, msParam_t *outParam, ruleExecInfo_t *rei)
{
    ...
    int iErr=0, i, continueInx, status, i2;
    transStat_t *transStat = NULL;
    genQueryInp_t genQueryInp;
    genQueryOut_t *genQueryOut = NULL;
    dataObjInp_t dataObjInp; 
    time_t t1;
    char strOut[MAX_NAME_LEN * MAX_NAME_LEN], *strAtrName, myGlbPar1[MAX_NAME_LEN]="";
    long lMin=55, lMax=100;
    int iCountMin = 0, iCountMax = 0, iCountMid = 0;

    int iCountUserDefinedMetadata = 0, iTotalEqualAvuAtr = (-1), iChkFailed = 0; 
    UserDefinedMetadata_t aAVUarray[1024];
 
    RE_TEST_MACRO ("    Calling msiChkDataAvuAttrOneValueOnly")

    printf("GJK- begin 0001.0.0k,Sat Mar  1 23:17:08 PST 2008, in msiChkDataAvuAttrOneValueOnly(), status=(%d), myCollInp->collName=(%s)\n", status, myCollInp->collName);
    ...
    if ((strAtrName = parseMspForStr (inpParam2)) != NULL)
      {
	lMin = 0; // lMin = strtol (strAtrName, (char **) NULL, 10);
      }
    else
      {
	sprintf (strOut,
	...
	return (-1);
      ...
      if (1 == 2)
	{
	  printf ("GJK fake11b STOP\n");
	  return (0);
	}
      else
	printf ("GJK fake11b OK\n");
...
/* iterate through all files */
    ...
    printf("GJK- begin 0001.0.1, in msiChkDataAvuAttrOneValueOnly(), status=(%d), myCollInp->collName=(%s)\n", status, myCollInp->collName);
    ...
    printf("GJKa2b myGlbPar1=(%s), myCollInp->collName=(%s)\n", myGlbPar1, myCollInp->collName);
      ...
      if (1 == 2)
	{
	  printf ("GJK fake12 STOP\n");
	  return (0);
	}
      else
	printf ("GJK fake12 OK\n");
    ...
    if (status < 0 && status != CAT_NO_ROWS_FOUND) {
      ...
      if (1 == 2)
	{
	  printf ("GJK fake00014k STOP, rei->status=(%d)\n", rei->status);
	  return (0);
	}
      else
	printf ("GJK fake00014k OK, rei->status=(%d)\n", rei->status);

    if (status != 0) {
      printf ("GJK 00014j OK, status=(%d) != 0\n", status);
      return(-1);
    }
    ...
    while (rei->status >= 0) {
      ...
      if (1 == 2)
	{
	  printf ("GJK fake00015k STOP, rei->status=(%d)\n", rei->status);
	  return (0);
	}
      else
	printf ("GJK fake00015k OK, rei->status=(%d)\n", rei->status);
      
      if (subColl == NULL) {
	printf ("GJK-P P.003.2.2b ERROR subColl == NULL\n");
	return(-1);
      }
      if (dataObj == NULL) {
	printf ("GJK-P P.003.2.2b ERROR dataObj == NULL\n");
	return(-1);
      }
      if (sqlDatSize == NULL) {
	...
	printf ("GJK-P P.003.2.2 OK sqlDatSize != NULL, dataObjInp.objPath=(%s), genQueryOut->rowCnt=(%d), i=(%d)\n", dataObjInp.objPath, genQueryOut->rowCnt, i);
      ...
      if (1 == 2)
	{
	  printf ("GJK fake00013k STOP\n");
	  return (0);
	}
      else
	printf ("GJK fake00013k OK\n");
	...
	printf ("GJK-P P.000300.33.1j genQueryOut->rowCnt=(%d), i=(%d), tmpSubColl=(%s), tmpDataName=(%s)\n", 
		genQueryOut->rowCnt, i, tmpSubColl, tmpDataName);
	
	if (1 == 2)
	  {
	    printf ("GJK fake1000 STOP\n");
	    return (0);
	  }
	else
	  printf ("GJK fake1000 OK\n");
...
#define no18
#ifdef no18
	{
	  ...
	  // /tempZone/home/rods/HELP.looptest
	  rstrcpy(myCollInp8.collName, dataObjInp.objPath, MAX_NAME_LEN);  /* rstrcpy(destination, source, max_len) */
	  rstrcpy(myCollInp8.collName, "/tempZone/home/rods/HELP.looptest", MAX_NAME_LEN);  /* rstrcpy(destination, source, max_len) */
	  status7 = intGetDataObjAVUsVol7 (&myCollInp8, aAVUarray, &iCountUserDefinedMetadata, strOut, rei);

	  for (i2 = 0; i2 <= iCountUserDefinedMetadata; i2++) {
	    printf ("GJK  0002000.0.1 iCountUserDefinedMetadata=(%d), aAVUarray[%d].attribute=[%s], i=(%d), status7=(%d)\n", 
		    iCountUserDefinedMetadata, i2, aAVUarray[i2].attribute, i, status7);
	    if (iTotalEqualAvuAtr == (-1)) 
	      iTotalEqualAvuAtr = status7;
	    else {
	      if (status7 != iTotalEqualAvuAtr)
		iChkFailed = 1;
	    }
	  }
	}
...
*/
      ...
      if (1 == 2)
	{
	  printf ("GJK fake1002 STOP\n");
	  return (0);
	}
      else
	printf ("GJK fake1002 OK\n");
    ...
    if (iChkFailed != 0)
      (void) snprintf (strOut, 255,
		       "Test failed, %d data objects VOL7, %d data objects are smaller than %ld bytes and %d data objects are larger than %ld bytes in the input '%s' collection\n", 
		       iCountMid, iCountMin, lMin, iCountMax, lMax, myGlbPar1);
    else
      (void) snprintf (strOut, 255,
		       "Test OK, %d data objects VOL7, %d data objects are smaller than %ld bytes and %d data objects are larger than %ld bytes in the input '%s' collection\n", 
		       iCountMid, iCountMin, lMin, iCountMax, lMax, myGlbPar1);
    ...
    i = fillStrInMsParam (outParam, strOut);     
      ...
      if (1 == 2)
	{
	  printf ("GJK fake1003 STOP\n");
	  return (0);
	}
      else
	printf ("GJK fake1003 OK\n");
 ...
 ruleExecInfo_t * rei)
{
  ...
  printf
    ("GJK-P P.21.0.1. in intGetDataObjAVUsVol6(), chrPtr1=(%s), ptrInpColl->collName=(%s)\n",
...
/*
  ...
  ("GJK-P P.21.1.1. in intGetDataObjAVUsVol6(), chrPtr1=(%s), ptrInpColl->collName=(%s), Pmath=%d, strlen=%d\n",
  ...
  */
  ...
  printf
    ("GJK-P P.21.0.2. in intGetDataObjAVUsVol6(), chrPtr1=(%s), ptrInpColl->collName=(%s)\n",
     chrPtr1, ptrInpColl->collName);

  if ((iI = isData (rei->rsComm, ptrInpColl->collName, NULL)) >= 0)
    {
      rodsLog (LOG_NOTICE,
	       "GJK intGetDataObjAVUsVol6: input (%s) is data.",
	       ptrInpColl->collName);
    }
  else
    {
      printf
	("GJK-P P.21.0.3. in intGetDataObjAVUsVol6(), chrPtr1=(%s), ptrInpColl->collName=(%s), iI=%d\n",
  ...
  printf ("GJK-P P.21.0.4. intGetDataObjAVUsVol6 : input (%s)\n", ptrInpColl->collName);
  ...
  printf
    ("GJK-P P.000914.0.12. in intGetDataObjAVUsVol6(), strAbsPath=(%s), ptrInpColl->collName=(%s), strDirName=(%s), strFileName=(%s), iErr=%d\n",
  ...
  printf
    ("GJK-P P.000914.0.13. in intGetDataObjAVUsVol6(), strAbsPath=(%s), ptrInpColl->collName=(%s), v3=(%s), iErr=%d\n",
  ...
  printf
    ("GJK-P P.0000914.0.14. in intGetDataObjAVUsVol6(), strAbsPath=(%s), ptrInpColl->collName=(%s), iErr=%d\n",
  ...
  printf
    ("GJK-P P.14.1.15. in intGetDataObjAVUsVol6(), strAbsPath=(%s), ptrInpColl->collName=(%s), iErr=%d, *iTotalAVUs=%d\n",
