	...
	for (i = 0; i < genQueryOut->rowCnt; i++)
	  {
	     ...
	     * dataId[NAME_LEN]; char                chksum[NAME_LEN]; char
	     * ownerName[NAME_LEN]; char                ownerZone[NAME_LEN];
...
#define BAD1
      ...
      for (i = 0; i < genQueryOut->rowCnt; i++)
	{
...

	  {
	     ...
	     * rescName[NAME_LEN];       / * This could
	     * be resource group * / char
	     * rescGroupName[NAME_LEN];       / * This
	     * could be resource group * / char
	     * dataType[NAME_LEN]; rodsLong_t dataSize;
	     * char chksum[NAME_LEN]; char
	     * version[NAME_LEN]; char
	     * filePath[MAX_NAME_LEN]; rescInfo_t
	     * *rescInfo; char dataOwnerName[NAME_LEN];
	     * char dataOwnerZone[NAME_LEN]; int replNum;
	     * int  replStatus;     / * isDirty flag * /
	     * char statusString[NAME_LEN]; rodsLong_t
	     ...
	     * dataAccess[NAME_LEN]; int dataAccessInx;
	     * char destRescName[NAME_LEN]; char
	     * backupRescName[NAME_LEN]; char
	   ...
	   * char dataId[NAME_LEN]; char chksum[NAME_LEN]; char
	   * ownerName[NAME_LEN]; char ownerZone[NAME_LEN];
	   * char createTime[TIME_LEN]; char
	   * modifyTime[TIME_LEN]; specColl_t *specColl; }
	   * rodsObjStat_t;
