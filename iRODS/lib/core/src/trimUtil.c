...
rodsArguments_t *rodsArgs, dataObjInp_t *dataObjInp)
{
	...
	sqlResult_t *subColl, *dataObj;
        ...
        if ((subColl = getSqlResultByInx (genQueryOut, COL_COLL_NAME))
          == NULL) {
            rodsLog (LOG_ERROR,
              "trimCollUtil: getSqlResultByInx for COL_COLL_NAME failed");
        ...
        if ((dataObj = getSqlResultByInx (genQueryOut, COL_DATA_NAME))
          == NULL) {
            rodsLog (LOG_ERROR,
              "trimCollUtil: getSqlResultByInx for COL_DATA_NAME failed");
            return (UNMATCHED_KEY_OR_INDEX);
        }
	...
	for (i = 0; i < genQueryOut->rowCnt; i++) {
	    ...
	    status = trimDataObjUtil (conn, srcChildPath,
